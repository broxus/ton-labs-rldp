use adnl::{
    common::{
        add_object_to_map, deserialize, get256, serialize, serialize_inplace, AdnlPeers, KeyId,
        Query, QueryId, Subscriber, Version,
    },
    dump,
    node::AdnlNode,
};
...
impl SendTransferState {
    ...
    fn set_part(&self, part: u32) {
        self.part
            .compare_and_swap(part - 1, part, Ordering::Relaxed);
    }
    fn set_reply(&self) {
    ...
    fn set_seqno_recv(&self, seqno: u32) {
        if self.seqno_sent() >= seqno {
            ...
            if seqno_recv < seqno {
                self.seqno_recv
                    .compare_and_swap(seqno_recv, seqno, Ordering::Relaxed);
            }
        }
    }
    fn set_seqno_sent(&self, seqno: u32) {
        ...
        if seqno_sent < seqno {
            self.seqno_sent
                .compare_and_swap(seqno_sent, seqno, Ordering::Relaxed);
        }
    }
}
...
impl RldpNode {
    ...
    fn answer_transfer(
        ...
        let inserted = add_object_to_map(&self.transfers, transfer_id.clone(), || {
        ...
        log::trace!(
            target: TARGET,
            "RLDP STAT recv: transfers total {}, active {}",
            all,
            now
        );
    ...
    async fn answer_transfer_loop(
        ...
        let answer = if let (true, answer) =
            Query::process_rldp(&subscribers, &query, &context.peers).await?
        {
    ...
    async fn query_transfer(
        ...
        let peer = if let Some(peer) = self.peers.get(peers.other()) {
            ...
            add_object_to_map(&self.peers, peers.other().clone(), || {
                let ret = RldpPeer {
        ...
        log::trace!(
            target: TARGET,
            "RLDP STAT send: transfers total {}, active {}",
            all,
            now
        );
        ...
        self.transfers
            .insert(recv_transfer_id.clone(), RldpTransfer::Recv(queue_sender));
        ...
        log::trace!(
            target: TARGET,
            "RLDP STAT send: transfers total {}, actual {}",
            all,
            now
        );
    ...
    async fn query_transfer_loop(
        ...
        self.transfers
            .insert(send_transfer_id.clone(), RldpTransfer::Done);
        if ok {
            log::trace!(
                target: TARGET,
